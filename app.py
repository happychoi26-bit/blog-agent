...
st.markdown(
    "다중 이미지 스마트 배치 + CTA 링크 자동 최적화 + 커스텀 요청사항 반영"
    " 엔진"
)
...
with st.sidebar:
  ...
  # ✨ 요구사항 반영: 카톡 말투 및 요즘 이모지 스타일 선택 옵션 개편
  ...
  st.markdown("---")
  st.markdown("### 🖼️ 다중 이미지 업로드 & 배치 전략")
  uploaded_images = st.file_uploader(
      "블로그에 삽입할 사진 여러 장 업로드 (다중 선택 가능)",
      type=["png", "jpg", "jpeg"],
  ...
  image_placement_mode = st.radio(
      "이미지 배치 방식 선택",
      [
          "🤖 업로드된 사진 개수에 맞춰 본문 전체에 자동 분산 배치",
          "📍 도입부 상단에 집중 배치",
...
def run_writer_agent(
  ...
  image_instruction = f"""
    [다중 이미지 삽입 가이드]
    - 사용자가 총 {image_count}장의 사진을 첨부했습니다.
    - 배치 방식: [{placement_mode}]
    - 원고 내에서 사진이 필요한 위치마다 `[📸 첨부 이미지 N번 삽입 위치 - 추천 이유: ...]` 마크를 위 배치 방식에 맞추어 총 {image_count}개 자연스럽게 삽입해 주세요.
    """
...
def analyze_seo_performance(article_text, company_name, region):
  ...
  return score, grade, f"{round(density_val, 1)}%"
...
if run_button:
  if not st.session_state.user_api_key:
    ...
    with st.status(
      ...
      st.session_state.competitor_analysis = run_competitor_analyzer_agent(
      ...
      st.session_state.research_data = run_researcher_agent(
          st.session_state.user_api_key, company_name, region, content_perspective
      )
      st.success("✔ 리서처 완료")

      st.write("✍️ [4/5] 톤 복제 라이터 에이전트: 커스텀 요청 반영 원고 작성 중...")
      ...
      st.write("🧩 [5/5] 어셈블러 에이전트: SEO 지표 정밀 평가 중...")
      score, grade, density = analyze_seo_performance(
...
if st.session_state.article_content is not None:
  ...
  with tab1:
    ...
    if uploaded_images:
      st.success(
          f"🖼️ **총 {len(uploaded_images)}장의 사진이 업로드되었으며,"
          f" [{image_placement_mode}] 규칙에 맞춰 원고 내에 최적 매칭되었습니다.**"
      )
  ...
  with tab4:
    st.markdown("### 📈 검색엔진 최적화(SEO) 및 리서치 지표")
